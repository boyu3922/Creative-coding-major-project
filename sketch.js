  ...
  draw() {
    for (let i = 0; i < this.points.length; i++) {
      ...
      if (p.z < 30) {
        ...
        let rgn = 67 + vertexOffset; // Number of vertices for star-like shape.
        if (rgn < 15) {
          vertexOffset = -52;
        }
        for (let j = 0; j < tt; j++) {
...
function keyPressed() {
  if (keyCode === LEFT_ARROW) { // Press left arrow to reduce the number of vertices.
    vertexOffset -= 16;
  } else if (keyCode === RIGHT_ARROW) { // Press right arrow to increase the number of vertices.
    vertexOffset += 16;
  }
